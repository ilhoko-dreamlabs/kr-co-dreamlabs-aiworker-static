# 7단계 worker.dreamlabs.co.kr 배포 전환 설계

## 목적

`kr-co-dreamlabs-aiworker-static` 저장소를 GitHub Pages 커스텀 도메인 `https://worker.dreamlabs.co.kr` 기준으로 배포할 수 있게 준비한다.
...
| 2 | 목표 production URL 확정 | 승인됨 | 완료: `https://worker.dreamlabs.co.kr` | 문서와 설정을 동일 기준으로 통일 | 사용자 지시 | 모든 문서에 목표 URL 반영 |
...
| 8 | production 배포 | 명시 승인됨 | 배포 시점과 롤백 커밋 확정 | GitHub Actions 배포 후 즉시 검증 | GitHub 반영 | `https://worker.dreamlabs.co.kr` 정상 응답 |
...
| 항목 | 결정 | 근거 |
|---|---|---|
| 운영 URL | `https://worker.dreamlabs.co.kr` | 사용자 명시 |
...
목표 URL: https://worker.dreamlabs.co.kr
결과: {완료/실패/보류}
검증: {실행한 검증}
다음 단계: {다음 작업}
승인 필요: {필요한 승인 또는 없음}
