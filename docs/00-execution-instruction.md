...
DreamLabs external 범위의 정적 프로젝트 `kr-co-dreamlabs-aiworker-static`를 `/workspace/github/kr-co-dreamlabs-aiworker-static`에서 운영 URL `https://worker.dreamlabs.co.kr` 기준으로 정비하고, 권고안대로 완료하라.
...
- 배포 플랫폼 기준: GitHub Actions 기반 GitHub Pages
...
- `public/CNAME`에 `worker.dreamlabs.co.kr`을 추가해 `dist/CNAME`이 생성되도록 한다.
- UI는 운영형, 신뢰 중심, 한국어 기본, 반응형, 접근성 고려 방향으로 구현한다.
- 홍보 랜딩이면서도 실제 활용 범위와 채널 연결을 바로 이해할 수 있는 정적 화면을 첫 화면으로 둔다.
...
| 구현 | 승인됨 | `worker.dreamlabs.co.kr` 기준 배포 준비 |
| 검증 | 승인됨 | typecheck/build |
| 실제 외부 배포 | 승인됨 | 사용자가 production 배포를 명시 지시함 |
